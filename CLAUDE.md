...
### 経路A: Chrome の Claude サイドパネル（通常の朝はこちら）

記事は Chrome の同じブラウザで読み、GitHub へはブラウザ操作経由で書き込む。
**ローカルにリポジトリのファイルは無い。** `git` コマンドも `python3` も使えない前提で動くこと。
Personal Access Token も不要。
...
4. GitHub 上で `articles/YYYY-MM-DD/<記事ID>.md` を **1 記事ずつ**作成する。
