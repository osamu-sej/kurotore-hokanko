...
1. Chrome で日経クロストレンドの新着一覧を開き、対象候補の URL とタイトルを控える。

2. **重複判定は GitHub 上のファイルの実在で行う。** ファイル名は URL から
   機械的に決まる（下の共通ルール参照）ので、次を開いて `<記事ID>.md` があるか見る。

   `https://github.com/osamu-sej/kurotore-hokanko/tree/main/articles/YYYY-MM-DD`

   **あれば取り込み済み。無ければ新規。** これが唯一の正しい判定方法。

   > **`index.json` を重複判定に使ってはいけない。**
   > あれは公開サイトのビルド結果なので、(a) push から反映まで 1〜2 分のラグがあり、
   > (b) ブラウザにキャッシュされる。取り込みの最中は必ず古い値を返すため、
   > 「リポジトリにあるのに index.json に無い」という食い違いが必ず起きる。
   > 2026-08-29 の取り込みで実際にこれが起き、索引の不具合と誤診された。
   > `index.json` は件数のざっくり確認にだけ使うこと。

   日付をまたいで探したい場合は、リポジトリ内検索で記事IDを引く:
   `https://github.com/search?q=repo:osamu-sej/kurotore-hokanko+<記事ID>&type=code`
...
- 連番を数えたり英語スラッグを考えたりしない。同じ記事なら常に同じファイル名になるので、
  取り込みを二度走らせても増殖しない。
- `order` はサイト上の表示順（同じ日付の中での並び）。**その日の既存ファイルの最大 order + 1** を付ける。
  その日の 1 本目だけ `order: 0` から始める。
  新着一覧の並び順は日中に記事が足されると変わるので、**一覧から採番し直してはいけない。**
  既存記事の order は書き換えず、後から来たものを末尾に足していく。
...
| `scripts/build_site.py` | `site/index.html` を生成（全記事を 1 ページに出力。左に検索とタグ絞り込み、右に日付見出しと記事カード） |
| `scripts/check_articles.py` | 必須項目・URL 重複・本文長を検査 |
| `scripts/gaps.py` | 取り込みの抜けと埋め戻しの進捗を一覧（週末・祝日は除外） |
...
移行分は `fetched_via: "artifact-migration"` で識別できる。

**トップページは全記事を 1 ページに出力する。** アーティファクト版と同じ見た目に揃えてあり、
月別ページや記事個別ページは無い。絞り込みはサーバー側で書き出し済みのカードに対して
JavaScript で行うので、JS が動かなくても記事は読める。

## 既知の欠落期間（バグではない）

**2026-08-03 〜 2026-08-25 の平日 17 日分が、ほぼ空になっている。**
この期間は保管庫のツール開発に時間を取られて取り込みが止まっていたためで、
データの破損でも集計漏れでもない。記事自体は存在するので、埋めるなら
通常の取り込みと同じ手順で遡って追加する（想定 約 218 本）。

平日の中央値は 5 本/日、直近の実績は 13 本/日程度。日によって本数がばらつくのは正常。

### 埋め戻しの手順

進捗は `python3 scripts/gaps.py` で確認できる（週末と祝日は除外済み。
「次に着手するなら」がそのまま次の対象になる）。経路A では実行できないので、
Osamu さんに出してもらうか、GitHub 上の `articles/<日付>/` を直接見て判断する。

**1 日分ずつ完了させてから次に進む。** 複数日を並行するとどこまでやったか分からなくなる。

1. 対象日の新着一覧を開く（その日に公開された記事の一覧）。
2. `articles/<対象日>/` にすでにあるファイルを確認する。**途中まで入っている日がある。**
3. 未取得のものだけを、通常の取り込みと同じ書式で作成する。
4. `order` は**その日の既存ファイルの最大 order + 1** から続ける。0 に戻さない。
5. `fetched_via` は `"claude-in-chrome"` にする（`artifact-migration` は移行分専用）。

日付が変わるだけで、書式もファイル名の決め方も通常の取り込みとまったく同じ。

---

`index.json` はサイト表示には使わない。全記事の一覧を機械可読で持っておくための索引で、
件数の確認などに使う。**取り込み時の重複判定には使わない**（ビルド結果なので必ず遅れる）。
